 ...
 * simplified NL99 network + hydrogen chemistry.
 * Use conservation of C and O, ignore M.
 * Add H chemistry in Glover+MacLow2007.
 * Update rates in Glover 2010.
 * TODO: add collisional dissociation of H2.
 */
...
class NL99p : public Ode {
  ...
  public:
    ...
    static int id(const std::string s) {
      SpecMap::const_iterator it = NL99p::spec_index_map.find(s);
      if (it == NL99p::spec_index_map.end()) {
        printf("spec=%s\n", s.c_str());
